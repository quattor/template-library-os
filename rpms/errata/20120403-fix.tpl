...
'/software/packages' = if (exists(SELF[escape('samba-common')]) || exists(SELF[escape('kdebase')])) {
    pkg_repl('libsmbclient','3.0.33-3.38.el5_8','x86_64');
} else {
    SELF;
};
...
'/software/packages' = if (exists(SELF[escape('tzdata')])) {
    pkg_del('tzdata');
    pkg_repl('tzdata','2012b-3.el5','x86_64');
} else {
    SELF;
};
...
'/software/packages' = if (exists(SELF[escape('xulrunner-devel')])) {
    ...
    pkg_repl('freetype-devel','2.2.1-28.el5_7.2','x86_64');
    ...
    pkg_repl('libpng-devel','1.2.10-16.el5_8','x86_64');
