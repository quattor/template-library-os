...
'/software/packages'= if (exists(SELF[escape('tzdata')])) {
    pkg_del('tzdata');
    pkg_repl('tzdata','2013c-2.el5','x86_64');
} else {
    SELF;
};
...
'/software/packages'= if (exists(SELF[escape('xulrunner-devel')])) {
    ...
    pkg_repl('mesa-libGL-devel','6.5.1-7.11.el5_9','x86_64');
} else {
    SELF;
};
...
'/software/packages'= {
	conflicts=list('','-cli','-common','-devel','-pdo','-mysql','-ldap','-odbc','-pgsql');
	
	if (exists(SELF[escape('php53-common')])) {
		_release=unescape(key(SELF[escape('php53-common')],0));
	    _arch=key(SELF[escape('php53-common')][escape(_release)]['arch'],0);
		foreach(k;v;conflicts) {
			_deprecated='php'+v;
			_newname='php53'+v;
...
# c-ares OS base version is newer than gLite-3.2
