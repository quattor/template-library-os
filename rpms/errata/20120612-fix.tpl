...
'/software/packages' = if (exists(SELF[escape('tzdata')])) {
    pkg_del('tzdata');
    pkg_repl('tzdata','2012c-1.el5',PKG_ARCH_DEFAULT);
} else {
    SELF;
};
...
'/software/packages' = if (exists(SELF[escape('xulrunner-devel')])) {
    ...
    pkg_repl('gnome-vfs2-devel','2.16.2-8.el5',PKG_ARCH_DEFAULT);
    pkg_repl('gtk2-devel','2.10.4-21.el5_5.6',PKG_ARCH_DEFAULT);
...
'/software/packages' = if (exists(SELF[escape('python')])) {
    pkg_repl('python-libs','2.4.3-44.el5',PKG_ARCH_DEFAULT);
} else {
    SELF;
};
